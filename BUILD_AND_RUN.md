...
### Frontend (front/.env)
```bash
# ВАЖНО: добавить /api так как все контроллеры используют Route("api/...")
...
- Проверьте файл `front/.env` - должен быть `VITE_API_URL=http://localhost:5000/api`
- Проверьте CORS настройки в `CarShareXAPI/Program.cs`
- **ВАЖНО:** URL должен заканчиваться на `/api` так как все контроллеры используют этот префикс
